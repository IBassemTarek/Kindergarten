...
String getFirstWord(String inputString) {
  ...
  Stream<UserData> initchildData () {
    return usersData.doc(uid).snapshots().map(_initchildDataFromSnapshot);
  }
UserData _initchildDataFromSnapshot(DocumentSnapshot snapshot) {
            return UserData( 
        name:"name" 
    );
}  
